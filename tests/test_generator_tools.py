...
def test_struct_generator():
    UID.reset()
    name, code = gen_struct(
        [("id", uint8_t), ("height", uint64_t), ("width", uint32_t)], name="teststruct"
    )
    assert name == "teststruct_uid1"
    assert (
        code
        == "struct teststruct_uid1{\n\tbit<8> id;\n\tbit<64> height;\n\tbit<32> width;\n}\n"
    )


def test_simple_state_generator():
    UID.reset()
    name, code = gen_simple_parser_state(
        "next_state", "target_header", [("pkt_type", uint8_t)], "check_custom"
    )
    assert name == "check_custom"
    generated_header_lines = code.get_headers().get_code().strip().split("\n")
    generated_parser_lines = code.get_parser().get_code().strip().split("\n")
    expected_header_lines = ["struct genstruct_uid1{", "\tbit<8> pkt_type;", "}"]
    expected_parser_lines = [
        "state check_custom{",
        "\tgenstruct_uid1 tmp = pkt.lookahead<genstruct_uid1>();",
        "",
        "\ttransition target_header;",
        "}",
    ]
    assert generated_header_lines == expected_header_lines
    assert generated_parser_lines == expected_parser_lines


def test_decision_state_generator():
    UID.reset()
    name, code = gen_decision_parser_state(
        [(SrcIp, "10.0.0.10"), (UdpDstPort, 5555), ("pkt_type", 7)],
        "next_state",
        "target_header",
        [("pkt_type", uint8_t)],
        "check_custom",
    ...
    assert name == "check_custom"
    assert (
        code.get_headers().get_code()
        == "struct genstruct_uid1{\n\tbit<8> pkt_type;\n}\n"
    )
    assert (
        code.get_parser().get_code()
...
def test_header_generator():
    UID.reset()
    name, handle, code = gen_header(
        [("id", uint8_t), ("height", uint64_t), ("width", uint32_t)]
    )
...
def test_flow_processor__just_headers():
    ...
    assert (
        gc.get_headers().get_code()
        == "header genhdr_uid1_h{\n\tbit<32> a;\n\tbit<32> b;\n}\nheader genhdr_uid2_h{\n\tbit<32> s;\n}\nheader genhdr_uid3_h{\n\tbit<32> t;\n}\n"
    )
    assert (
        gc.get_hdrlist().get_code()
...
def test_generate_chain():
    ...
    f2 = FlowSelector("IPV4_UDP", [(UdpDstPort, 7777)], fp)

    assert (
        generate_chain("IPV4_UDP", [fs, f2]).get_parser().get_code()
        == "state check_uid4{\n\ttransition select(hdr.udp.dstPort){\n\t\t(7777) : parse_genhdr_uid1;\n\t\tdefault: accept;\n\t}\n}\nstate check_uid5{\n\ttransition select(hdr.udp.dstPort){\n\t\t(5555) : parse_genhdr_uid1;\n\t\tdefault: check_uid4;\n\t}\n}\n#define CHAIN_IPV4_UDP\nstate chain_ipv4_udp{\n\ttransition check_uid5;\n}\n"
    )


test_simple_state_generator()
