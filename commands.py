...
class AssignConst(Command):
    ...
    def get_generated_code(self):
        ...
        gc.get_apply().writeln(
            "{} = {};".format(vi["handle"], vi["type"].to_p4_literal(self.value))
        )
        return gc
...
class Increment(Command):
    ...
    def get_generated_code(self):
        ...
        gc.get_apply().writeln(
            "{} = {} + {};".format(
                vi["handle"], vi["handle"], vi["type"].to_p4_literal(self.value)
            )
        )
        return gc
...
class StrictAddition(StrictNumericTwoOperandCommand):
    def get_generated_code(self):
        ...
        gc.get_apply().writeln(
            "{} = {} + {};".format(t["handle"], a["handle"], b["handle"])
        )
        return gc

    def execute(self, test_env):
        ti = self.env.get_varinfo(self.target)
        test_env[self.target] = ti["type"].cast_value(
            test_env[self.operand_a] + test_env[self.operand_b]
        )
...
class AssignRandomValue(Command):
    ...
    def check(self):
        var_exists(self.vname, self.env)
        assert self.env.get_varinfo(self.vname)["type"] in [
            uint8_t,
            uint16_t,
            uint32_t,
            uint64_t,
        ], "Not supported random generation"
        target_type = self.env.get_varinfo(self.vname)["type"]
        assert target_type.cast_value(self.min_value) < target_type.cast_value(
            self.max_value
        ), "Max is not greater than max. (Check for overflows too)"
    ...
    def execute(self, test_env):
        target_type = self.env.get_varinfo(self.vname)["type"]
        test_env[self.vname] = random.randint(
            target_type.cast_value(self.min_value),
            target_type.cast_value(self.max_value),
        )
...
class Switch(Command):
    ...
    def get_generated_code(self):
        ...
        gc.get_apply().writeln(
            "if ({}=={}){{".format(
                self.env.get_varinfo(self.vname)["handle"],
                self.env.get_varinfo(self.first_guess)["handle"],
            )
        )
        ...
        while actual_block.get_next_block() != None:
            ...
            if next_guess != None:
                gc.get_apply().writeln(
                    "else if ({}=={}){{".format(
                        self.env.get_varinfo(self.vname)["handle"],
                        self.env.get_varinfo(next_guess)["handle"],
                    )
                )
            else:
                gc.get_apply().writeln("else{")
    ...
    def execute(self, test_env):
        ...
        while actual_block != None and (
            actual_guess != None and value != test_env[actual_guess]
        ):
...
class SetStandardField(Command):
    ...
    def get_generated_code(self):
        ...
        gc.get_apply().writeln(
            "{} = {};".format(
                self.field.get_handle(), self.field.get_type().to_p4_literal(self.value)
            )
        )
        return gc
...
class CastVar(Command):
    ...
    def get_generated_code(self):
        ...
        t = self.env.get_varinfo(self.target)
        gc.get_apply().writeln(
            "{} = ({}) {};".format(t["handle"], t["type"].get_p4_type(), s["handle"])
        )
        return gc
